...
import music21
from harte_utils import calculate_interval, grammar_rule_to_music21_chord_type
...
MISSING_INTERVALS = {
    ...
    "sharp-ninth-13th": "1,3,5,-7,#9,11,13",
    "flat-13th": "1,3,5,-7,9,11,-13",
    ...
    "flat-ninth-flat-13th": "1,3,5,-7,-9,11,-13",
    ...
    "seventh-sharp-9th-flat-5th": "1,3,-5,-7,#9",
    "major-ninth-sharp-11th": "1,3,5,7,9,#11",
    "diminished-seventh-major-seventh": "1,-3,-5,7",
    "seventh-flat-9th-flat-5th": "1,3,-5,-7,-9",
    "flat-ninth-sharp-13th": "1,3,5,-7,-9,11,#13",
    "flat-13th-sharp-11th": "1,3,5,-7,9,#11,-13",
    "suspended-flat-thirteen": "1,4,5,-7,11,-13",
    "flat-ninth-13th": "1,3,5,-7,-9,11,13",
    "sharp-eleventh-13th": "1,3,5,-7,9,#11,13",
    "flat-fifth-9th": "1,3,-5,-7,9",
    "sharp-ninth-sharp-13th": "1,3,5,-7,#9,11,#13",
    "sharp-ninth-flat-13th": "1,3,5,-7,#9,11,-13",
    "dominant-flat-ninth": "1,3,5,-7,-9,11,13",
    "augmented-minor-seventh": "1,-3,#5,-7",
    "dominant-sharp-ninth": "1,3,5,7,#9",
    "augmented-major-9th-sharp-11th": "1,3,#5,-7,9,11",
...
class HarteTransformer(Transformer):
    ...
    @staticmethod
    def _build_chord_repr(
        root: str, shorthand: str, intervals: List[str], bass: str
    ) -> str:
        """
        Parameters
        ----------
    ...
    def chord(self, chord_constituents: List) -> str:
        ...
        if len(chord_constituents) > 0 and "/" in chord_constituents[-1]:
            ...
            if len(alternate_bass) > 1:
                if alternate_bass[0] == "b" and alternate_bass[0].islower():
                    alternate_bass = alternate_bass[0].upper() + alternate_bass[
                        1:
                    ].replace("b", "-").lstrip("+")
                else:
        ...
        return self._build_chord_repr(
            root, shorthand, sorted(list(intervals)), alternate_bass
        )
